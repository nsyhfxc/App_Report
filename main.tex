\documentclass[12pt]{ctexart}
\usepackage{config}

%LaTeX模板作者：GitHub@nsyhfxc
%请勿修改config.sty的设置,否则会导致格式混乱


\newcommand{\SoftwareName}{XXXXXXXX软件} %填软件名称
\newcommand{\DocType}{设计说明} %这里区分是设计说明还是使用说明
\newcommand{\Version}{V1.0} %版本号是V1.0
\newcommand{\Authors}{XXXX} %作者姓名
\newcommand{\CreatDate}{\today} %文档创建时间
\newcommand{\UpdateDate}{\today} %文档更新时间

%这里是全局文本设置,相关文本仅需设置一次即可


\begin{document}


\input{chapter/index.tex} %包括封面设置、修订记录、目录

\input{chapter/mainbody.tex} %包括主体的全部内容

\end{document}